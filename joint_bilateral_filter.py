...
if __name__ == '__main__':
    sigma_s = 5
    sigma_r = 0.1*255
    img = cv2.imread('sky1_noisy.png')
    #img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    img_gray = cv2.imread('sky1_guide_color.png')
    img_gray = cv2.cvtColor(img_gray, cv2.COLOR_BGR2GRAY)
    tic = time.time()
    img_bf = bilateralfilter(img, img_gray, sigma_s, sigma_r)
    toc = time.time()
    print('Elapsed time: %f sec.' % (toc - tic))
    cv2.imwrite('sky1_guide_gray.png', img_gray)
    cv2.imwrite('sky1_filtered_image.png', img_bf)
